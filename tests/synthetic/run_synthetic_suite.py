...
def _collect_run_metrics(
    ...
    metrics: dict[str, Any] = {
        ...
        "parity_true": _classify_parity_truth(meta),
...
def _evaluate_test(
    ...
    if kind == "no_knee":
        ...
        regime_rate = float(
            ((run_df["slope_delta"].abs() >= regime_delta).fillna(False)).mean()
        )
...
def _classify_parity_truth(meta: dict[str, Any]) -> str:
...
def _classify_band_truth(meta: dict[str, Any], truth: dict[str, Any], model_name: str) -> str:
    ...
    if parity_true == "null":
        return "incoherent"
    if model_name in {"hybrid_knee", "piecewise_forbidden"}:
...
def _classify_band_hat(metrics: dict[str, Any]) -> str:
    band = metrics.get("band_class_hat")
...
def _evaluate_calibration(
    ...
    for col in ("gamma_abs_pct_err", "xi_abs_pct_err", "omega_k_abs_pct_err"):
    ...
    for key in ("gamma_abs_pct_err", "xi_abs_pct_err", "omega_k_abs_pct_err"):
        ...
        cat_rate = float((vals > catastrophic_cutoff).mean())
